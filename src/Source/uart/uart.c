...
extern ULONG SystemFrequency; /*!< System Clock Frequency (Core Clock)  */

volatile ULONG UART0Status, UART1Status, UART2Status;
volatile UCHAR UART0TxEmpty = 1, UART1TxEmpty = 1, UART2TxEmpty = 1;
volatile UCHAR UART0Buffer[BUFSIZE], UART1Buffer[BUFSIZE],
		UART2Buffer[BUFSIZE];
volatile ULONG UART0Count = 0, UART1Count = 0, UART2Count = 0;

volatile xQueueHandle UART0Queue;
volatile xQueueHandle UART1Queue;
volatile xQueueHandle UART2Queue;
 ...
 **                      been received
 ** Returned value:		character received from serial port or -1 (ULONG)
 **
 *****************************************************************************/
ULONG UARTGetChar(ULONG portNum, ULONG blocking) {
	xQueueHandle queue;
	UCHAR ch;
...
void UART2_IRQHandler(void) {
	...
	if (IIRValue == IIR_RLS) /* Receive Line Status */
	{
		...
		if (LSRValue & LSR_RDR) /* Receive Data Ready */
		{
			/* If no error on RLS, normal ready, save into the data buffer. */
			/* Note: read RBR will clear the interrupt */
			rdChar = LPC_UART2 ->RBR;
			UART2Buffer[UART2Count] = rdChar;
			if (UART2Queue) {
				xQueueSendFromISR(UART2Queue, &rdChar, pdFALSE);
			}
			UART2Count++;
 ...
 **
 ** parameters:			portNum(0, 1 or 2) and UART baudrate
 ** Returned value:		true or false, return false only if the
 **						interrupt handler can't be installed to the
 **						VIC table
 **
 *****************************************************************************/
ULONG UARTInit(ULONG PortNum, ULONG baudrate) {
	ULONG Fdiv;
	ULONG pclkdiv, pclk;
 ...
 **
 *****************************************************************************/
void UARTSend(ULONG portNum, UCHAR *BufferPtr, ULONG Length) {
	if (portNum == 0) {
		while (Length != 0) {
 ...
 **
 *****************************************************************************/
void UARTSendNullTerminated(ULONG portNum, UCHAR *BufferPtr) {
	if (portNum == 0) {
		while (*BufferPtr != 0) {
			/* THRE status, contain valid data */
			while (!(UART0TxEmpty & 0x01))
